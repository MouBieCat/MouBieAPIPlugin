...
import com.cat.moubieapi.api.yaml.FileSection;
...
public abstract class FileModuleAbstract
     ...
     */
    public FileModuleAbstract(final @NotNull Plugin plugin, final @NotNull String path, final @NotNull String name, final boolean isNewFile) {
        super(plugin, path);

        final File pathObj = new File(plugin.getDataFolder(), path);
        final File fileObj = new File(pathObj, name);

        if (!fileObj.exists()) {
            if (isNewFile) {
                if (!this.createFile(fileObj))
                    Debugger.warning(this.plugin, "§c創建一個新的檔案配置失敗，這是一個錯誤訊息。");
            }
            else
    ...
    public final void save(final @NotNull File file) {
        try {
            ...
            e.printStackTrace();
            Debugger.warning(this.plugin, "§c嘗試將檔案保存至實體位置失敗，這是一個錯誤訊息。");
        }
    }
